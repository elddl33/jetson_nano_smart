...
source=ROOT / '1'  # file/dir/URL/glob/screen/0(webcam)
...
def run():
    ...
    while True:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if count_1 == 0:
                thread = Thread(target=gen_frame, args=())
                print("Thread2 on")
                thread.daemon = True
                thread.start()
                count_1 = 1
...
def gen_frame():
    global frame
    while True:
        im0_resize = cv2.resize(im0, (160, 120))
        retval, buffer = cv2.imencode('.jpg', im0_resize, [int(cv2.IMWRITE_JPEG_QUALITY), 100])
        #retval, buffer = cv2.imencode('.jpg', im0, [int(cv2.IMWRITE_JPEG_QUALITY), 100])
...
@app.route("/")
def index():                            
    return render_template("index_2.html") # Render the web page template
...
if __name__ == "__main__":
    ...
    thread.daemon = True
    thread.start()
    app.run(host="192.168.0.141", port=8000)#, debug=True)
    #app.run(host="192.168.70.200", port=8000, debug=True)
